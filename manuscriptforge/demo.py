...
def run_demo(output_dir: Path | str) -> dict[str, Any]:
    ...
    write_text(project / "DEMO_REPORT.md", (
        ...
        "This original synthetic writing collection, or corpus, describes an invented distance-sensor "
        "documentation procedure. It contains no actual measurements and runs without a model provider.\n\n"
        f"Three documents in Markdown, text, and DOCX produced {summary['extracted_chunks']} chunks, "
        "meaning extracted, section-labeled passages. Curation records inclusion and exclusion decisions: "
        "the demo retained six passages and explicitly excluded the DOCX methods duplicate. "
        "Both copies carry a repeated_boilerplate warning; that warning alone does not remove text. "
        "The registry, a table of passages and review decisions, was rebuilt and checked before profiling.\n\n"
        f"The style profile summarizes {summary['profile_word_count']} words and "
        ...
        f"The {summary['style_cards']} style cards provide compact section and overall reports with source-linked examples; "
        "open the [overall style card](planning/intake/style_cards/academic_manuscript__overall.md). "
